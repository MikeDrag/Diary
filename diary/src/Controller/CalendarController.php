    ...
    public function index()
    {
            ...
            if ($count_days_of_month <= $last_week_previous_month) {
                for ($count_previous_month_days = 0; $count_previous_month_days < $last_day_of_month_last_count; $count_previous_month_days++) {
                    ...
                    $diary .= '<td style="opacity:0.4" data-toggle="modal" data-target="#myModal" data-id="' . ($last_day_of_month_last_count - $last_week_previous_month + $count_previous_month_days) . '">' .
	                    '<a href="diary.php/d=' . ($last_day_of_month_last_count - $month_start + $days_diff_previous_month) .  '&m=' . date('m') . '"/>'
                ...
                if ($day_of_the_month == $count_days) {
                    ...
                    $diary .= '<td data-toggle="modal" data-target="#myModal" data-id="' . $count_days . '">' . '<a href="diary.php/d=' . $count_days . '&m=' . date('m') . '"/>' . $count_days . '</a></td>';
                }
            }
	    ...
	    for ($remaining_days = 0; $remaining_days < $days_removed; $remaining_days++) {
		    ...
		    if (date('l', strtotime($count_days . date('-m-Y'))) === 'Sunday') {
			    $diary .= '<td data-toggle="modal" data-target="#myModal data-id="' . $count_days . '">' . '<a href="d=' . $count_days . '&m=' . date('m') . '"/>' . $count_days . '</a></td>' . '<tr>';
		    } else {
			    $diary .= '<td data-toggle="modal" data-target="#myModal data-id="' . $count_days . '">' . '<a href="d=' . $count_days . '&m=' . date('m') . '"/>' . $count_days . '</a></td>';
		    }
	    }
	    ...
	    for ($last_week_next_month; $last_week_next_month <= 7; $last_week_next_month++){
		    ...
		    $diary .= '<td style="opacity:0.5" data-toggle="modal" data-target="#myModal">' . '<a href="diary.php/d=' . 1 . '&m=' . $date = date('m', strtotime('+1 month')) . '"/>' . '0' . $count_next_month_last_week_days. '</a></td>';
	    }
